...
join (
  values
    ('football', 'Premier League (Test)', 'pilot-premier-league', 'GB', 'league'),
    ('football', 'Bundesliga (Test)', 'pilot-bundesliga', 'DE', 'league'),
    ('basketball', 'NBA (Test)', 'pilot-nba', 'US', 'league'),
    ('tennis', 'Grand Slam (Test)', 'pilot-grand-slam', 'FR', 'tournament'),
    ('cricket', 'International Cricket (Test)', 'pilot-cricket-intl', 'IN', 'series'),
    ('athletics', 'Track Series (Test)', 'pilot-track-series', 'AU', 'series')
...
-- Attach official external URL for the live_external pilot card only (metadata).
update public.sports_fixtures
set metadata = metadata || jsonb_build_object(
  'officialUrl', 'https://www.example.com/official-broadcast'
)
where metadata->>'source' = 'sports_private_pilot_2026_07_18'
  and metadata->>'pilot_kind' = 'external_official'
  and availability_state = 'live_external';
