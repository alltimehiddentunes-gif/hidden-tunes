...
export async function GET(request: NextRequest) {
  ...
  try {
    const result = q
      ? await listAudiobooks({ page, limit, searchQuery: q, canAccessMature })
    ...
    logAudiobookError("Failed to search audiobooks.", error);
    return jsonAudiobookError("Failed to search audiobooks.", 500, error);
  }
}
