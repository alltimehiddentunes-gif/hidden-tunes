...
import { clampArtistPageSize, jsonArtistError, loadArtistCredits } from "@/lib/artistCatalog";
...
export async function GET(request: NextRequest, context: ArtistRouteContext) {
  ...
  try {
    ...
    const items = await loadArtistCredits(resolved.artistId, limit);
    return NextResponse.json({ success: true, items });
  } catch (error) {
    return artistErrorResponse(error, "Failed to load artist credits.");
  }
}
