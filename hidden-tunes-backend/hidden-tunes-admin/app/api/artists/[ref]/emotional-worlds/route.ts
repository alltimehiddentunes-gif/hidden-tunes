...
import { clampArtistPageSize, jsonArtistError, loadArtistEmotionalWorlds } from "@/lib/artistCatalog";
...
export async function GET(request: NextRequest, context: ArtistRouteContext) {
  ...
  try {
    ...
    const items = await loadArtistEmotionalWorlds(resolved.artistId, limit);
    return NextResponse.json({ success: true, items });
  } catch (error) {
    return artistErrorResponse(error, "Failed to load emotional worlds.");
  }
}
