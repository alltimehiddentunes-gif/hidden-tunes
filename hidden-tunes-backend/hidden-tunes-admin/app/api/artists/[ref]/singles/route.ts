...
import { clampArtistPageSize, jsonArtistError, loadArtistSingles } from "@/lib/artistCatalog";
import {
  artistErrorResponse,
  artistListResponse,
  ArtistRouteContext,
...
export async function GET(request: NextRequest, context: ArtistRouteContext) {
  ...
  try {
    ...
    const page = await loadArtistSingles(resolved.artistId, { limit, cursor: query.cursor });
    return artistListResponse(page.items, page);
  } catch (error) {
    return artistErrorResponse(error, "Failed to load artist singles.");
  }
}
