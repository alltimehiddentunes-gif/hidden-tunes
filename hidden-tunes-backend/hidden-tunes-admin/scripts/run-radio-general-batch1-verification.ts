...
const candidatePath = path.join(adminRoot, "data", "radio-general-batch1-candidates.json");
const checkpointPath = path.join(adminRoot, "data", "radio-general-batch1-verification-checkpoint.json");
const resultPath = path.join(adminRoot, "data", "radio-general-batch1-verification-result.json");
...
async function recordRun(supabase: SupabaseClient, report: Record<string, unknown>) {
  ...
  const { error } = await supabase.from("radio_import_runs").upsert(
    {
      run_id: `radio-general-batch1-verification-${report.mode}`,
