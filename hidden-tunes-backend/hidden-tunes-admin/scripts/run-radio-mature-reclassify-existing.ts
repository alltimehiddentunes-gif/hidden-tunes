...
async function loadCandidates(supabase: SupabaseClient, limit: number, pageSize: number) {
  ...
  while (rows.length < limit) {
    ...
    const { data, error } = await supabase
      .from("radio_stations")
      .select(
        "id, name, tags, homepage_url, country_code, language, stream_url, is_mature, playback_status, is_verified, reliability_score, quality_score, quarantined_at, disabled_at"
      )
      .eq("is_mature", false)
...
async function main() {
  ...
  for (const row of candidates) {
    ...
    // Keep already-public playable rows searchable after reclassification.
    // Leaving mature_source_approved=false drops them from frozen-mobile /search
    // until a separate promote pass runs.
    const alreadyPublicEligible =
      row.playback_status === "playable" &&
      row.is_verified === true &&
      !row.quarantined_at &&
      !row.disabled_at &&
      Number(row.reliability_score ?? row.quality_score ?? 0) >= 60;
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        mature_source_approved: alreadyPublicEligible,
        is_free: true,
