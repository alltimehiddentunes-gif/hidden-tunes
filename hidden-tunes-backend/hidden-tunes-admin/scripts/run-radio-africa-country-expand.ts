/**
 * Africa Radio country expansion — one country at a time on the existing pipeline.
 *
 * Usage:
 *   npx tsx scripts/run-radio-africa-country-expand.ts --country GH --execute
 *   npx tsx scripts/run-radio-africa-country-expand.ts --country NG --execute
 *   npx tsx scripts/run-radio-africa-country-expand.ts --seed-queue
 *   npx tsx scripts/run-radio-africa-country-expand.ts --status
 */
...
const USER_AGENT = "HiddenTunes/1.0 africa-country-expand";

type Mode = "dry-run" | "execute";

type AfricaCandidate = NormalizedRadioStation & {
...
function readArgs() {
  ...
  return {
    ...
    maxPagesPerQuery:
      maxPagesIndex >= 0 ? Number(process.argv[maxPagesIndex + 1]) : Number(process.env.AFRICA_MAX_PAGES || 80),
    verifyLimit:
      verifyLimitIndex >= 0
        ? Number(process.argv[verifyLimitIndex + 1])
        : Number(process.env.AFRICA_VERIFY_LIMIT || 0) || null,
...
function reportPath(code: string) {
  return path.join(adminRoot, "data", "radio-africa-reports", `${code.toLowerCase()}-expand-report.json`);
}

function checkpointPath(code: string) {
  return path.join(adminRoot, "data", "radio-africa-reports", `${code.toLowerCase()}-candidates.json`);
}
...
async function fetchQueryPage(query: RadioExpansionQuery, offset: number, options: ReturnType<typeof readArgs>) {
...
async function discoverCountry(
  ...
  const candidates: AfricaCandidate[] = [];
      ...
      try {
        ...
        for (const raw of stations) {
          ...
          if (normalized.country_code && normalized.country_code !== countryCode) {
            // Keep only rows that resolve to this country code.
            continue;
          }
          if (!normalized.country_code) {
            normalized.country_code = countryCode;
            if (!normalized.country) {
              const meta = getAfricanCountry(countryCode);
              normalized.country = meta?.name || countryCode;
            }
          }
          if (
            isMatureRadioCandidate({
          ...
          // Keep in-memory catalog warm so later queries in this run dedupe.
        ...
        // If a name/language query yields no in-country hits for a full page, stop early.
  ...
  return { candidates, sourcesSearched: Array.from(new Set(sourcesSearched)), exhausted, failedPages };
}

async function importCandidates(
  supabase: SupabaseClient,
  candidates: AfricaCandidate[],
  options: ReturnType<typeof readArgs>
) {
  const stats = { imported: 0, duplicates: 0, failed: 0, dry_run_would_insert: 0, insertedIds: [] as string[] };
...
async function loadCountryVerifyRows(supabase: SupabaseClient, code: string, forceAllFailed: boolean) {
      ...
      try {
        ...
        if (options.mode === "execute") {
          ...
          if (probe.playable && probe.finalUrl) {
            // Prefer stable https final URL when probe resolved one; keep original for http relay path.
            ...
            payload.delivery_mode = String(probe.finalUrl).startsWith("https://")
              ? "direct_https"
              : "backend_relay";
            if (probe.contentType) payload.codec = row.name ? undefined : undefined;
          }
...
function printStatus(entry: AfricaCountryQueueEntry) {
  console.log(
    JSON.stringify(
      {
        code: entry.code,
...
async function main() {
  ...
  if (options.seedQueue || options.statusOnly) {
    if (options.seedQueue) {
      // Refresh baseline public counts for every country.
      ...
      for (const country of AFRICA_RADIO_QUEUE) {
        ...
        entry.public_playable_total = await publicPlayableCount(supabase, country.code);
        entry.mature_public_total = await maturePublicCount(supabase, country.code);
      }
      ...
      console.log(
        JSON.stringify(
          {
            seeded: true,
            path: path.join(adminRoot, "data", "radio-africa-expansion-queue.json"),
  ...
  if (!options.country) {
    throw new Error("Pass --country GH (or another ISO code), or --seed-queue / --status");
  }

  const meta = getAfricanCountry(options.country);
  if (!meta) throw new Error(`Country ${options.country} is not in the Africa queue`);
  ...
  const entry = getQueueEntry(queue, meta.code)!;
  ...
  // Phase A — repair existing failed/quarantined/unchecked
  let repairStats = { tested: 0, playable: 0, restored: 0, quarantined: 0, rejected: 0, updated: 0, outcomes: {} };
  ...
  // Phase B — deep discover
  let candidates: AfricaCandidate[] = [];
  if (!options.skipDiscover) {
    const catalogIndex = await loadCatalogDedupeIndex(supabase);
    const queries = buildAfricaCountryQueries(meta);
    const discovery = await discoverCountry(meta.code, queries, options, catalogIndex);
    candidates = discovery.candidates;
    ensureDir(checkpointPath(meta.code));
    fs.writeFileSync(
      checkpointPath(meta.code),
  ...
  } else if (fs.existsSync(checkpointPath(meta.code))) {
    const saved = JSON.parse(fs.readFileSync(checkpointPath(meta.code), "utf8")) as { candidates: AfricaCandidate[] };
    candidates = saved.candidates || [];
    entry.candidates_discovered = candidates.length;
  }

  // Phase C — import unique new stations
  let importStats = { imported: 0, duplicates: 0, failed: 0, dry_run_would_insert: 0, insertedIds: [] as string[] };
  ...
  // Phase D — verify unchecked / newly imported for this country
  let verifyStats = { tested: 0, playable: 0, restored: 0, quarantined: 0, rejected: 0, updated: 0, outcomes: {} };
  ...
  if (afterPublic === beforePublic && importStats.imported === 0 && Number(repairStats.restored || 0) === 0) {
    entry.notes = [...(entry.notes || []), "No public playable delta this run; sources may be exhausted or all candidates failing verification."];
  }
  queue.current_country_code = meta.code;
  saveAfricaRadioQueue(adminRoot, queue);

  ensureDir(reportPath(meta.code));
  fs.writeFileSync(reportPath(meta.code), JSON.stringify(report, null, 2));
  printStatus(entry);
  console.log(JSON.stringify({ report_path: reportPath(meta.code), summary: report }, null, 2));
}
