...
import { buildRadioExpansionBatch11Queries } from "@/lib/radioExpansion25k/batch11Queries";
import { buildRadioExpansionBatch12Queries } from "@/lib/radioExpansion25k/batch12Queries";
import { buildRadioExpansionBatch13Queries } from "@/lib/radioExpansion25k/batch13Queries";
import { buildRadioExpansionBatch14Queries } from "@/lib/radioExpansion25k/batch14Queries";
import { buildRadioExpansionBatch15Queries } from "@/lib/radioExpansion25k/batch15Queries";
import { buildRadioExpansionBatch16Queries } from "@/lib/radioExpansion25k/batch16Queries";
import { buildRadioExpansionBatch17Queries } from "@/lib/radioExpansion25k/batch17Queries";
import { buildRadioExpansionBatch18Queries } from "@/lib/radioExpansion25k/batch18Queries";
import { buildRadioExpansionBatch19Queries } from "@/lib/radioExpansion25k/batch19Queries";
import {
...
import {
  ...
  RADIO_EXPANSION_BATCH11_NAME,
  RADIO_EXPANSION_BATCH12_NAME,
  RADIO_EXPANSION_BATCH13_NAME,
  RADIO_EXPANSION_BATCH14_NAME,
  RADIO_EXPANSION_BATCH15_NAME,
  RADIO_EXPANSION_BATCH16_NAME,
  RADIO_EXPANSION_BATCH17_NAME,
  RADIO_EXPANSION_BATCH18_NAME,
  RADIO_EXPANSION_BATCH19_NAME,
...
const EMPTY_PAGE_EXHAUST_THRESHOLD = Number(process.env.RADIO_BATCH_EMPTY_STREAK || 5);
...
function buildExpansionQueriesForBatch(batchName: string) {
  ...
  if (batchName === RADIO_EXPANSION_BATCH11_NAME) {
    return buildRadioExpansionBatch11Queries();
  }
  if (batchName === RADIO_EXPANSION_BATCH12_NAME) {
    return buildRadioExpansionBatch12Queries();
  }
  if (batchName === RADIO_EXPANSION_BATCH13_NAME) {
    return buildRadioExpansionBatch13Queries();
  }
  if (batchName === RADIO_EXPANSION_BATCH14_NAME) {
    return buildRadioExpansionBatch14Queries();
  }
  if (batchName === RADIO_EXPANSION_BATCH15_NAME) {
    return buildRadioExpansionBatch15Queries();
  }
  if (batchName === RADIO_EXPANSION_BATCH16_NAME) {
    return buildRadioExpansionBatch16Queries();
  }
  if (batchName === RADIO_EXPANSION_BATCH17_NAME) {
    return buildRadioExpansionBatch17Queries();
  }
  if (batchName === RADIO_EXPANSION_BATCH18_NAME) {
    return buildRadioExpansionBatch18Queries();
  }
  if (batchName === RADIO_EXPANSION_BATCH19_NAME) {
    return buildRadioExpansionBatch19Queries();
  }
