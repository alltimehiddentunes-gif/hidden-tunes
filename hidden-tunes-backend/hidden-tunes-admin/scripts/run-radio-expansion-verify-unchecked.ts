...
async function loadUncheckedStationIds(
  ...
  for (;;) {
    const { data, error } = await supabase
      ...
      .eq("is_mature", false)
      .is("disabled_at", null)
      // Oldest first: newest imports are often long-tail / broken RB leftovers.
    ...
    for (;;) {
      ...
      if (options.mode === "execute") {
        try {
          ...
          // Drain progress: don't leave retryable failures in unchecked forever.
          if (!probe.playable && update.playback_status === "unchecked") {
            ...
            await supabase
              ...
              .update({
                ...
                consecutive_failures: Math.max(1, Number(update.consecutive_failures || 1)),
                last_health_checked_at: nowIso,
                last_health_error: probe.reason,
                quarantined_at: nowIso,
                quarantine_reason: probe.outcome || "expansion_verify_failed",
              })
              .eq("id", row.id);
            stats.quarantined += 1;
          } else if (update.quarantined_at) {
            stats.quarantined += 1;
          }
          stats.updated += 1;
        } catch {
