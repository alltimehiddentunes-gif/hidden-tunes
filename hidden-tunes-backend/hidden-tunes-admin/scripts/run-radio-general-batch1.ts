...
const checkpointPath = path.join(adminRoot, "data", "radio-general-batch1-candidates.json");
const resultPath = path.join(adminRoot, "data", "radio-general-batch1-result.json");
const RADIO_BROWSER_SERVERS = [
  "https://de1.api.radio-browser.info",
  "https://all.api.radio-browser.info",
] as const;
const USER_AGENT = "HiddenTunes/1.0 radio batch1";
...
function readArgs() {
  ...
  return {
    mode,
    target: targetIndex >= 0 ? Number(process.argv[targetIndex + 1]) : 5000,
...
async function recordImportRun(supabase: SupabaseClient, report: Record<string, unknown>) {
  ...
  const { error } = await supabase.from("radio_import_runs").upsert(
    {
      run_id: `radio-general-batch1-${report.mode}`,
