export type RadioExpansionQueryKind =
  ...
  | "tag_votes_asc"
  | "tag_clicks"
  | "country_include_broken"
  | "global_include_broken"
  | "tag_include_broken"
  | "language_include_broken";
...
export function buildRadioBrowserPath(
  ...
  if (query.kind === "country_votes_asc") {
    return `/json/stations/bycountrycodeexact/${encodeURIComponent(
      query.value
    )}?${common}&order=votes&reverse=false`;
  }
  if (query.kind === "country_clicks") {
    return `/json/stations/bycountrycodeexact/${encodeURIComponent(
      query.value
    )}?${common}&order=clickcount&reverse=true`;
  }
  if (query.kind === "country_lastcheck") {
    return `/json/stations/bycountrycodeexact/${encodeURIComponent(
      query.value
    )}?${common}&order=lastchecktime&reverse=true`;
  }
  if (query.kind === "tag_votes_asc") {
    return `/json/stations/search?tag=${encodeURIComponent(
      query.value
    )}&${common}&order=votes&reverse=false`;
  }
  if (query.kind === "tag_clicks") {
    return `/json/stations/search?tag=${encodeURIComponent(
      query.value
    )}&${common}&order=clickcount&reverse=true`;
  }
  if (query.kind === "country_include_broken") {
    return `/json/stations/bycountrycodeexact/${encodeURIComponent(
      query.value
    )}?limit=${safeLimit}&offset=${safeOffset}&hidebroken=false&order=votes&reverse=true`;
  }
  if (query.kind === "global_include_broken") {
    return `/json/stations/search?limit=${safeLimit}&offset=${safeOffset}&hidebroken=false&order=lastchecktime&reverse=true`;
  }
  if (query.kind === "tag_include_broken") {
    return `/json/stations/search?tag=${encodeURIComponent(
      query.value
    )}&limit=${safeLimit}&offset=${safeOffset}&hidebroken=false&order=votes&reverse=false`;
  }
  if (query.kind === "language_include_broken") {
    return `/json/stations/search?language=${encodeURIComponent(
      query.value
    )}&limit=${safeLimit}&offset=${safeOffset}&hidebroken=false&order=votes&reverse=false`;
  }
