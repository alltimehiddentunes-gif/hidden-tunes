...
export function resolvePodcastMatureFields(input: {
  title?: string;
  categories?: string[];
  ...
  const fromApi = Boolean(input.is_mature);
  const fromCategory = Boolean(input.categoryIsMature);
  const fromCategories = categories.some((entry) =>
    /\badult\b/i.test(String(entry))
  );
  const fromTitle = inferMatureFromName(String(input.title || ""));
  const isMature = fromApi || fromCategory || fromCategories || fromTitle;
  ...
  return {
    ...
    mature_reason:
      ...
      (fromCategory
        ? "mature_category"
        : fromCategories
          ? "mature_show_category"
          : fromTitle
            ? "mature_title"
            : undefined),
    content_rating: isMature
      ? parseContentRating(input.content_rating || "adult")
      : parseContentRating(input.content_rating),
  };
}
