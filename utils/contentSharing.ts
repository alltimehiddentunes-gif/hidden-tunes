...
function segment(value: string, label?: string): string {
  return encodeURIComponent(requireStableContentId(value, label));
}

export function canonicalPathForContent(content: ShareableContent): string {
  if (content.type === 'podcastEpisode') {
    return `/podcasts/${segment(content.showId, 'podcast ID')}/episodes/${segment(content.episodeId, 'episode ID')}`;
  }
  if (content.type === 'audiobookChapter') {
    return `/audiobooks/${segment(content.bookId, 'audiobook ID')}/chapters/${segment(content.chapterId, 'chapter ID')}`;
  }
...
export function shareMessageForContent(content: ShareableContent): string {
  const title = content.title.trim() || 'Hidden Tunes';
  const suffix = canonicalUrlForContent(content);
...
function decodedSegment(value: string): string | null {
  try {
    ...
    requireStableContentId(decoded);
    return decoded;
  } catch {
    return null;
  }
...
export function parseHiddenTunesUrl(value: string): ParsedHiddenTunesContent | null {
  ...
  if (parts.length === 4 && parts[0] === 'podcasts' && parts[2] === 'episodes') {
    const showId = decodedSegment(parts[1]);
    const episodeId = decodedSegment(parts[3]);
    return showId && episodeId ? { type: 'podcastEpisode', showId, episodeId } : null;
  }
  if (parts.length === 4 && parts[0] === 'audiobooks' && parts[2] === 'chapters') {
    const bookId = decodedSegment(parts[1]);
    const chapterId = decodedSegment(parts[3]);
  ...
  const type = parts.length === 2
    ...
    : parts.length === 3 && parts[0] === 'radio' && parts[1] === 'stations'
      ? 'radioStation'
      : parts.length === 3 && parts[0] === 'tv' && parts[1] === 'channels'
        ? 'tvChannel'
        : null;
