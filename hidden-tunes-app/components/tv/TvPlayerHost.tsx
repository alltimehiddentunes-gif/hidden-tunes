...
/**
 * Single WebView host. Presentation mode only changes layout styles —
 * the WebView stays on a stable tree path so floating ↔ full does not remount.
 */
function TvPlayerHost({
  ...
  const body = (
    <>
      ...
      {/* Stable path: Root → Card → videoShell → WebView (never reparented) */}
      <View
        style={[
          styles.videoShell,
  ...
  // Stable tree: root → bgSlot → card → videoShell → WebView.
