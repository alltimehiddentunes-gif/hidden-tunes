...
export default function MotivationSpeakersScreen() {
  const insets = useSafeAreaInsets();
  const [query, setQuery] = useState("");
  const speakers = useMemo(() => {
    const all = listStashedMotivationEntities("speaker").sort((a, b) =>
      a.displayName.localeCompare(b.displayName, undefined, { sensitivity: "base" })
    );
    const q = query.trim().toLowerCase();
    if (!q) return all;
    return all.filter((speaker) => speaker.displayName.toLowerCase().includes(q));
  }, [query]);

  const renderItem = useCallback(
    ({ item }: { item: (typeof speakers)[number] }) => (
      <TouchableOpacity
        style={styles.row}
        onPress={() =>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>
              Open Motivationals home first to load speaker previews, then return here.
            </Text>
          }
        />
      </LinearGradient>
