...
assert.match(playbackBridge, /const targetVolume = muted \? 0 : Math\.max\(0, Math\.min\(1, volume\)\)/);
assert.match(playbackBridge, /hiddenAudioVolumeWriteSequence/);
assert.match(playbackBridge, /\.then\(\(\) => hiddenAudioBridge\.setVolume\(targetVolume\)\)/);
