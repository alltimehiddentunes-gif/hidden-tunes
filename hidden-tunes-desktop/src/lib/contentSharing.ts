...
const containsForbiddenIdCharacters = (value: string) => /[\\/]/.test(value) || Array.from(value).some((character) => {
...
export function requireStableContentId(value: string, label = 'content ID'): string {
  const id = value.trim()
  if (!id || id.length > 200 || containsForbiddenIdCharacters(id)) throw new Error(`Invalid ${label}`)
  let decoded = id
  ...
  if (!decoded || decoded === '.' || decoded === '..' || containsForbiddenIdCharacters(decoded)) throw new Error(`Invalid ${label}`)
  return id
}
