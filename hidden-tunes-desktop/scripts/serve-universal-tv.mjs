...
const artworkOrigin = 'https://pub-cdc7ab995ca34ff1b3f95453a8024aa3.r2.dev'
const artworkMaxBytes = 8 * 1024 * 1024
const provenArtworkPath = '/covers/1786560058995-5716e869-VOY-A-LLeVARTE-PA-PR-Nate-The-DJ-Club-Edit-Bad-Bunny.jpg'
...
async function getStaticArtwork(name) {
  ...
  let source
  if (name === 'player.jpg') {
    const upstream = await fetch(`${artworkOrigin}${provenArtworkPath}`, { redirect: 'manual', headers: { Accept: 'image/jpeg' } })
    if (!upstream.ok || upstream.status >= 300) throw new Error('static-upstream-failed')
    source = await readBoundedBody(upstream)
  } else if (name === 'fallback.jpg') {
    source = readFileSync(join(root, 'brand', 'hidden-tunes-official.png'))
  } else throw new Error('static-name-rejected')
  const result = await convertForVidaa(source, name === 'fallback.jpg' ? 384 : 720)
...
createServer(async (request, response) => {
  ...
  if (pathname === '/__tv_diag') {
    const allowed = ['event', 'kind', 'keyCode', 'selected', 'owner', 'before', 'after', 'width', 'height'].map((key) => `${key}=${String(requestUrl.searchParams.get(key) || '').replace(/[^A-Za-z0-9_. -]/g, '').slice(0, 48)}`).join(' ')
  ...
  if (pathname === '/__tv_art/player.jpg' || pathname === '/__tv_art/fallback.jpg') {
    const method = request.method || 'GET'
