...
/** Category labels only — used when /api/tv/categories is unreachable. No playable URLs. */
export function buildTvBrowseCategoryFallback(): TvBrowseCategory[] {
  const categories: TvBrowseCategory[] = TV_BROWSE_CATEGORY_NAMES.map((name) => ({
    id: slugify(name),
    name,
    slug: slugify(name),
    parentSlug: null,
...
> = {
  ...
  faith: [{ category: "Faith & Worship" }, { category: "Worship Music" }],
};
