...
class HiddenAudioModule(
  ...
  fun updateRemoteQueueAvailability(activeIndex: Int, queueLength: Int, promise: Promise) {
    mainHandler.post {
      try {
        val data = Arguments.createMap()
        data.putInt("activeIndex", activeIndex)
        data.putInt("queueLength", queueLength)
        emitDiagnostic("android_auto_queue_synced", data)
        promise.resolve(null)
      } catch (error: Throwable) {
        emitDiagnostic("android_auto_media_session_error", errorData(error))
        promise.reject("ANDROID_AUTO_QUEUE_SYNC_FAILED", error)
      }
    }
  }
  ...
  fun syncAndroidAutoCatalog(snapshot: ReadableMap, promise: Promise) {
    mainHandler.post {
      try {
        ...
        emitDiagnostic("android_auto_visible_check")
        promise.resolve(null)
      } catch (error: Throwable) {
        emitDiagnostic("android_auto_media_session_error", errorData(error))
