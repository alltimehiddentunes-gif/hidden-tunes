...
object HiddenAudioCore {
    ...
    exoPlayer.addListener(object : Player.Listener {
      ...
      override fun onPlayerError(error: PlaybackException) {
        ...
        if (
          ...
          emitDiagnostic("network_dns_failure_track_invalidated", data)
        }
        emitDiagnostic(
  ...
  fun playForcedFromSession() {
    ...
    emitDiagnostic(
      "android_auto_command_blocked_no_native_track",
      simpleData("reason", "no_loaded_native_track_or_js_context")
    )
  }

  fun skipToNextFromSession() {
    emitAutoDiagnostic("android_auto_next_command_received")
    val context = reactContext
    if (context != null && context.hasActiveReactInstance()) {
  ...
  fun skipToPreviousFromSession() {
    emitAutoDiagnostic("android_auto_previous_command_received")
    val context = reactContext
    if (context != null && context.hasActiveReactInstance()) {
  ...
  private fun syncMediaSession() {
    ...
    val data = Arguments.createMap()
    data.putString("title", track?.title ?: "Hidden Tunes")
    data.putString("artist", track?.artist ?: "Hidden Tunes")
    data.putBoolean("hasArtwork", !track?.artworkUrl.isNullOrBlank())
    data.putString("status", playerStatus)
    emitDiagnostic("android_auto_metadata_synced", data)
  }
